...
class SupConLoss(nn.Module):
    ...
    def forward(self, features, labels=None, temperature=0.07, mask=None):
        ...
        if labels is not None and mask is not None:
            ...
            labels = labels.contiguous().view(-1, 1) #将 labels 的形状调整为 (batch_size, 1)
            if labels.shape[0] != batch_size:
                raise ValueError('Num of labels does not match num of features')
            mask = torch.eq(labels, labels.T).float().to(device) #对比度掩码，mask_{i,j}=1 if sample j has the same class as sample i
        else:
